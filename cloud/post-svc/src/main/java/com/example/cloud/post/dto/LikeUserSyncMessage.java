...
public class LikeUserSyncMessage {
    ...
    public static LikeUserSyncMessage deserialize(String payload) {
        if (payload == null || payload.isEmpty()) throw new IllegalArgumentException("payload is empty");
        String[] parts = payload.split("\\|");
        if (parts.length != 4) throw new IllegalArgumentException("payload format invalid");
